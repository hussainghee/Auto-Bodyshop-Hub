...
import { api, fmtDate, waLink } from "../lib/api";
...
const emptyForm = {
  name: "", mobile: "", email: "", address: "", notes: "", preferred_contact: "mobile",
  vehicles: [],
};
...
export default function Customers() {
  ...
  const updateVehicle = (i, patch) => {
    const next = form.vehicles.map((v, idx) => idx === i ? { ...v, ...patch } : v);
  ...
  const save = async (e) => {
    ...
    try {
      const vehicles = form.vehicles
        ...
        .map(v => ({
          vehicle_type: v.vehicle_type, make: v.make === "__other__" ? "Other" : v.make,
  ...
  return (
    <div data-testid="customers-page">
      <PageHeader title="Customers" subtitle="CRM"
        actions={<Button onClick={() => { setForm(emptyForm); setOpen(true); }} className="bg-[#0066FF] hover:bg-[#3385FF] rounded-sm" data-testid="new-customer-btn"><Plus size={16} className="mr-1.5" /> New Customer</Button>}
      />
      <div className="p-8 space-y-4">
        <div className="relative max-w-md">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Search by name, mobile, email…" value={q} onChange={e => setQ(e.target.value)} className="pl-9 bg-[#0F1115] border-border rounded-sm h-10" data-testid="customer-search" />
        </div>
        ...
        <div className="border border-border bg-[#0F1115] rounded-sm overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody data-testid="customer-table">
              {list.length === 0 && <tr><td colSpan={6} className="px-6 py-12 text-center text-muted-foreground">No customers found.</td></tr>}
              {list.map(c => (
                <tr key={c.id} className="border-b border-border/60 hover:bg-white/[0.02]" data-testid={`customer-row-${c.id}`}>
                  ...
                  <td className="px-6 py-3 font-mono-data">
                    <div className="flex items-center gap-2">
                      {c.mobile}
                      <a href={waLink(c.mobile)} target="_blank" rel="noreferrer" title="WhatsApp" className="text-emerald-400 hover:text-emerald-300" data-testid={`wa-${c.id}`}><MessageCircle size={14} /></a>
                    </div>
                  </td>
          ...
          <form onSubmit={save} className="space-y-4 mt-2" data-testid="customer-form">
            ...
            <div className="border border-border rounded-sm">
              <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Vehicles (optional, can add later)</div>
              ...
              <div className="divide-y divide-border">
                {form.vehicles.length === 0 && <div className="px-4 py-6 text-center text-sm text-muted-foreground">No vehicles yet. Click "Add Vehicle" to attach one or more.</div>}
                {form.vehicles.map((v, i) => (
                  <div key={i} className="p-4 grid grid-cols-2 md:grid-cols-4 gap-3 relative" data-testid={`inline-vehicle-${i}`}>
                    <button type="button" onClick={() => removeVehicle(i)} className="absolute top-2 right-2 text-muted-foreground hover:text-[#FF3B30]" data-testid={`remove-vehicle-${i}`}><Trash2 size={14} /></button>
                    <div>
                    ...
                    <MakeModelSelect
                      make={v.make}
                      model={v.model}
                      onChange={({ make, model }) => updateVehicle(i, { make, model })}
                    />
