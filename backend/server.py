...
def _normalize_whatsapp_number(mobile: Optional[str]) -> str:
    """
    Meta WhatsApp Cloud API expects the country code and number without '+'.
    For Kuwait local 8-digit numbers, default to country code 965.
    """
...
def _get_integration_field(fields: Dict[str, Any], *names: str) -> Optional[str]:
    """
    Safely reads integration fields saved by the CRM settings screen.
    Supports snake_case and label-style keys.
    """
    if not fields:
        return None
...
async def _get_whatsapp_config() -> Dict[str, str]:
    settings = await db.system_settings.find_one({"_id": "singleton"}, {"_id": 0}) or {}

    toggles = settings.get("toggles") or {}
    if not toggles.get("whatsapp_enabled"):
        raise HTTPException(400, "WhatsApp integration is disabled in system settings.")

    cfg = ((settings.get("integrations") or {}).get("whatsapp") or {})
    if not cfg.get("enabled"):
        raise HTTPException(400, "WhatsApp integration configuration is not enabled.")

    fields = cfg.get("fields") or {}

    api_base_url = _get_integration_field(
        fields,
        "api_base_url",
        "base_url",
        "api url",
    ...
    access_token = _get_integration_field(
        fields,
        "access_token",
        "token",
        "Access Token",
    ...
    phone_number_id = _get_integration_field(
        fields,
        "phone_number_id",
        "sender",
    ...
    quotation_template = _get_integration_field(
        fields,
        "quotation_template",
        "template_quotation_share",
        "quotation_share_template",
        "Template — Quotation Share",
        "Template - Quotation Share",
    ) or "hello_world"

    invoice_template = _get_integration_field(
        fields,
        "invoice_template",
        "template_invoice_share",
        "invoice_share_template",
        "Template — Invoice Share",
        "Template - Invoice Share",
    ) or "hello_world"

    if not access_token:
        raise HTTPException(400, "WhatsApp access token is missing.")

    if not phone_number_id:
        raise HTTPException(400, "WhatsApp phone number ID is missing.")

    return {
        "api_base_url": api_base_url.rstrip("/"),
        "access_token": access_token,
        "phone_number_id": phone_number_id,
        "quotation_template": quotation_template.strip(),
        "invoice_template": invoice_template.strip(),
    }
...
async def _send_whatsapp_template(
    ...
    wa_to = _normalize_whatsapp_number(to_mobile)

    if not wa_to:
        raise HTTPException(400, "Customer mobile number is missing or invalid.")

    cfg = await _get_whatsapp_config()
    ...
    # Meta's default test template hello_world does not accept custom parameters.
    if template_name.lower() != "hello_world" and body_parameters:
    ...
    url = f'{cfg["api_base_url"]}/{cfg["phone_number_id"]}/messages'
    ...
    try:
        ...
        raise HTTPException(502, f"Could not connect to WhatsApp API: {exc}") from exc
    ...
    if res.status_code >= 400:
        try:
            ...
            err = {"error": res.text}
        raise HTTPException(res.status_code, f"WhatsApp API error: {err}")
...
async def send_quotation_whatsapp(qid: str, user=Depends(require_permission("quotations"))):
    ...
    cfg = await _get_whatsapp_config()
    template_name = cfg.get("quotation_template") or "hello_world"

    result = await _send_whatsapp_template(
        customer.get("mobile"),
        template_name,
        body_parameters=[
            customer.get("name") or "",
            q.get("number") or "",
            f'KWD {float(q.get("total", 0) or 0):.3f}',
        ],
    )
    ...
    await db.quotations.update_one(
        ...
        {
            "$set": {
                "whatsapp_last_sent_at": now_iso(),
                "whatsapp_last_sent_by": user["id"],
                "updated_at": now_iso(),
                "updated_by": user["id"],
            }
        },
    ...
    return {
        "ok": True,
        "message": "Quotation WhatsApp message sent.",
        "template": template_name,
        "to": _normalize_whatsapp_number(customer.get("mobile")),
...
async def send_job_invoice_whatsapp(jid: str, user=Depends(require_permission("jobs"))):
    ...
    cfg = await _get_whatsapp_config()
    template_name = cfg.get("invoice_template") or "hello_world"

    document_number = job.get("invoice_number") or job.get("number") or ""
    status_label = (job.get("status") or "").replace("_", " ")
    paid, balance_due, payment_status = _job_payment_summary(job)
    ...
    result = await _send_whatsapp_template(
        ...
        body_parameters=[
            ...
            document_number,
            status_label,
            f'KWD {float(job.get("total", 0) or 0):.3f}',
            f'KWD {float(balance_due or 0):.3f}',
        ],
    )
    ...
    return {
        "ok": True,
        "message": "Invoice WhatsApp message sent.",
        "template": template_name,
        "to": _normalize_whatsapp_number(customer.get("mobile")),
        "provider_response": result,
    }
...
async def add_payment(jid: str, body: PaymentIn, user=Depends(require_permission("jobs"))):
    ...
    payment_amount = round3(float(body.amount or 0))
    ...
    await db.jobs.update_one(
        {"id": jid},
        {
            ...
            "$set": {"updated_at": now_iso(), "updated_by": user["id"]},
        },
    )
    return _enrich_job(await db.jobs.find_one({"id": jid}, {"_id": 0}))
